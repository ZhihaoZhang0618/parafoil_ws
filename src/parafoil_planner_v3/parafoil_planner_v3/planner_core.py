...
class PlannerCore:
            ...
            for k in range(len(controls)):
                ...
                if k > 0:
                    ...
                    if dt > 1e-6:
                        ...
                        if rate_l > float(self.config.delta_rate_max) or rate_r > float(self.config.delta_rate_max):
    ...
    def plan(
        ...
        if landing_site_selection is not None:
            self.last_site_selection = landing_site_selection
            planning_target = landing_site_selection.target
        ...
        if self.config.use_library and (
